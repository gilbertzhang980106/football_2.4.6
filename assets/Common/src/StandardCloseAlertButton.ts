...
export default class StandardCloseAlertButton extends cc.Component {
        ...
        if (this.clickSound || this.clickInvalid) {
            ...
            btn['_onTouchEnded'] = (e) => {
                if (this.clickInvalid) {
                    ...
                    //发送返回主页的请求
                    DznSocket.emit(gameData.messageFlag.HIDE_ALERT_LAYER);
                }
                if (this.clickSound) {
                    AudioManage.playEffect(audioConfig.effect_button_close);
                }
                _onTouchEnded(e);
            }
        }
