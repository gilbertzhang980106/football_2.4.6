 ...
 */
@ccclass
export default class LList extends LListAbs {
    ...
    onLoad() {
        ...
        if (this.bg && this.autoSize) {
            let x = 0 - this.bg.x;
            let y = this.bg.y - 0;
            let w = this.bg.width - this.node.width - x;
            let h = this.bg.height - this.node.height - y;
            this.fixSize = new cc.Rect(x, y, w, h);
        }

        // if(this.bg) this.bg.active = false;

        this.scrollView.enabled = this.enabledScroll as boolean;
    }
    ...
    public setData(itemDataList: Array<any>) {
        ...
        if (this.none) this.none.active = (itemDataList.length == 0);
        this.updateContent();
    ...
    private reSize() {
        ...
        let mask: cc.Node = this.scrollView.node.getChildByName("view");
    ...
    private getItem() {
        if (this.itemPool.length == 0) {
            let item: cc.Node = cc.instantiate(this.itemRender)
    ...
    private itemClickHandler(event: cc.Event) {
        let item = (event.currentTarget as cc.Node).getComponent(ItemRender);
        // console.log("itemClickHandler ", item.itemIndex);
        this._selectedItem = item.data;
        this._selectedIndex = item.itemIndex;
        this.node.dispatchEvent(new LEvent(LEvent.ITEM_CLICK, item.itemIndex))
    }
    ...
    /**获取选中的视图*/
    public getSelectedItemrander():cc.Node {
        if (this._selectedIndex < 0) return null;
        let itemR: ItemRender;
        for (let item of this.content.children) {
            itemR = item.getComponent(ItemRender);
            if (itemR && itemR.itemIndex == this.selectedIndex) return item;
        }
        return null;
    }
