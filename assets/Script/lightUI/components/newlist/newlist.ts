 ...
 */
@ccclass
export default class newlist extends cc.Component {
    ...
    private itemClickHandler(event: cc.Event) {
        let item = (event.currentTarget as cc.Node).getComponent(listItemRender);
    ...
    public getSelectedItemrander(): cc.Node {
        ...
        let itemR: listItemRender;
        for (let item of this.content.children) {
            itemR = item.getComponent(listItemRender);
    ...
    private updateContent() {
        ...
        if (this.itemList.length == 0) {
            ...
            var startIndex = this.itemList[0].getComponent(listItemRender).itemIndex;
